...
# ignore ruff E731
def preamble(model_name=MODEL_NAME, \
                    date_str="today", \
                    mlc_open="\"\"\"", \
                    mlc_close="\"\"\"") \
: return f"""{mlc_open}
...
def parse_ignore_patterns(ignore_str: Optional[str]) -> List[re.Pattern]:
    ...
    for pattern in ignore_str.split(','):
        ...
        if ptrn:
            ...
            regex_pattern = re.compile(glob.fnmatch.translate(ptrn))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate unit tests " +
                                     "for Python code using Gemini API")

    parser.add_argument("--api_key", type=str,
                        help="API Key for Gemini (can also be set via " +
                        "GEMINI_API_KEY environment variable)")
    parser.add_argument("path", type=str,
                        help="Path to the Python file or directory to generate unit tests for")
    parser.add_argument("--mirror", action="store_false",
                        help="Mirror the output directory structure")
    parser.add_argument("--verbose", action="store_true",
                        help="Print verbose output")
    parser.add_argument("--dry_run", action="store_true",
                        help="Dry run (no API calls or file writing)")
    parser.add_argument("--ignore", type=str,
                        help="Comma-separated list of glob patterns to " +
                        "ignore (e.g. 'setup.py,*_pb2.py')")
    parser.add_argument("--no-preamble", action="store_true",
                        help="Do not include the test generation preamble")
    parser.add_argument("-r", "--recursive", action="store_true",
                        help="Recursively generate unit tests for all files in a directory")
    parser.add_argument("-f", "--force", action="store_true",
                        help="Force overwrite of existing unit tests")
    parser.add_argument("-o", "--output", type=str,
                        help="Output directory for generated unit tests " +
